...
public class Dash_GenerarReporte_Asistencia implements ActionListener {
    
    private generar_reporte_asistencia genRepAs_view;
    private Evento modelEvento;
    private DashboardDAO dash_DAO;

    public Dash_GenerarReporte_Asistencia(generar_reporte_asistencia genRepAs_view, Evento modelEvento) {
        this.genRepAs_view = genRepAs_view;
        this.modelEvento = modelEvento;
        dash_DAO = new DashboardDAOImpl();
        
        cargarEventoEnComboBox();
        
        genRepAs_view.btn_generarreporte_evento.addActionListener(this);
        genRepAs_view.btn_cancelar_reportevento.addActionListener(this);
    ...
    {
        if(e.getSource() == genRepAs_view.btn_generarreporte_evento)
        {
            Connection conexion = (Connection) ConexionSQLWorkbrench.obtenerConexion();
            JasperReport reporte = null;
            String path="src\\reportes\\reporte_asistencia\\ReportAsistencia.jasper";

            Map parametro = new HashMap();
            int idEvento = Integer.parseInt(genRepAs_view.txt_idevento.getText());
    ...
    private void cargarEventoEnComboBox() {
        ...
        genRepAs_view.cbx_evento.setModel(new DefaultComboBoxModel<>());
        // Agrega cada EVENTO al JComboBox solo con el NOMBRE
        for (Evento evento : eventos) {genRepAs_view.cbx_evento.addItem(evento.getNombreEvento());}
        // Agrega un ActionListener al JComboBox para manejar eventos de selección
        genRepAs_view.cbx_evento.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                // Obtén el ítem seleccionado del JComboBox (que incluye el apellido)
                String nombreSeleccionado = (String) genRepAs_view.cbx_evento.getSelectedItem();
                ...
                genRepAs_view.txt_idevento.setText(String.valueOf(idEvento));
            }
        });
    }
