...
public class Dash_GenerarReporte_Factura  implements ActionListener  {
    
    private generar_reporte_factura genRepFa_view;
    private Evento modelEvento;
    private DashboardDAO dash_DAO;

    public Dash_GenerarReporte_Factura(generar_reporte_factura genRepFa_view, Evento modelEvento) {
        this.genRepFa_view = genRepFa_view;
        this.modelEvento = modelEvento;
        dash_DAO = new DashboardDAOImpl();
        
        cargarEventoEnComboBox();
        
        genRepFa_view.btn_generarreporte_evento.addActionListener(this);
        genRepFa_view.btn_cancelar_reportevento.addActionListener(this);
    ...
    {
        if(e.getSource() == genRepFa_view.btn_generarreporte_evento)
        {
            Connection conexion = (Connection) ConexionSQLWorkbrench.obtenerConexion();
            JasperReport reporte = null;
            String path="src\\reportes\\reporte_factura\\ReportFactura.jasper";

            Map parametro = new HashMap();
            int idEvento = Integer.parseInt(genRepFa_view.txt_idevento.getText());
    ...
    private void cargarEventoEnComboBox() {
        ...
        genRepFa_view.cbx_evento.setModel(new DefaultComboBoxModel<>());
        // Agrega cada EVENTO al JComboBox solo con el NOMBRE
        for (Evento evento : eventos) {genRepFa_view.cbx_evento.addItem(evento.getNombreEvento());}
        // Agrega un ActionListener al JComboBox para manejar eventos de selección
        genRepFa_view.cbx_evento.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                // Obtén el ítem seleccionado del JComboBox (que incluye el apellido)
                String nombreSeleccionado = (String) genRepFa_view.cbx_evento.getSelectedItem();
                ...
                genRepFa_view.txt_idevento.setText(String.valueOf(idEvento));
            }
        });
    }
